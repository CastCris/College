...
class Captcha():
    ...
    def client_token_key_gen()->str|None:
        pass

    @Class.property
    def client_token_key(cls)->str|None:
        return cls.client_token_key_gen()

    @Class.property
    def client_token_key_valid(cls)->bool:
        try:
            return not cls.client_token_key is None
        except:
            return False
    ...
    @Class.property
    def client_token(cls)->str|None:
        from begin.globals import r

        # print('client_token valid: ', cls.client_token_key_gen)
        if not cls.client_token_key_valid:
            return

        key = cls.client_token_key
        # print('client_token key: ', r.get(key), key)
        return r.get(key)


    ##
    @classmethod
    def client_token_del(cls)->None:
        from begin.globals import r
    ...
    @classmethod
    def client_token_save(cls, token:str)->None:
        ...
        key = cls.client_token_key
        validity = cls.TOKEN_VALIDITY

        r.set(key, Crypt.argon2_crypt(token))
        r.expire(key, validity)

    @classmethod
    def client_token_auth(cls, token_input:str)->bool:
        from begin.globals import Crypt

        token = cls.client_token
        print('token_auth: ', token, cls.client_token)
        return Crypt.argon2_auth(token, token_input)

    ##
    @staticmethod
    def code_img_generate()->str:
        from begin.globals import Crypt
        import string
